...
function nextTokenValue(tokens: Token[], current: number, direction: 1 | -1): number | null {
  const sorted = tokens.toSorted((a, b) => a.value - b.value);
  if (direction > 0) {
    const t = sorted.find((t) => t.value > current + 0.5);
    return t ? t.value : null;
  } else {
    const t = sorted.toReversed().find((t) => t.value < current - 0.5);
    return t ? t.value : null;
  }
}
...
export function BudgeMePaperPreview({
  ...
  autoFocus,
}: { features?: PreviewFeatures; autoFocus?: boolean } = {}) {
  ...
  const [pressedButton, setPressedButton] = useState<
    "reset" | "copy" | "prev" | "next" | "mute" | null
  >(null);
  ...
  const toastDismissTimeoutRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    toastLabelTimeoutRef.current = setTimeout(() => {
      ...
      toastDismissTimeoutRef.current = setTimeout(() => setToastLabel(null), 300);
    }, 1400);
    return () => {
      clearTimeout(toastLabelTimeoutRef.current);
      clearTimeout(toastDismissTimeoutRef.current);
    ...
    (direction: number, shift = false, held = false) => {
      ...
      if (snapToTokensRef.current && cs.tokens && !shift) {
        ...
        if (tokenNext !== null) {
          ...
          const sorted = cs.tokens.toSorted((a, b) => a.value - b.value);
  ...
  return (
    <>
      <div ref={wrapperRef} className="relative">
        ...
        >
          <div
            className={`flex min-h-0 flex-col items-center grow shrink basis-[0%]${f.showText === false && f.showLabel === false ? " justify-center" : ""}`}
          >
            {f.showLabel !== false && (
              ...
              <div className="hidden sm:block absolute bottom-4 right-4">
                ...
                >
                  <svg
                    ...
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    style={{ width: 18, height: "auto", flexShrink: 0 }}
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M5 7C3.343 7 2 8.343 2 10V19C2 20.657 3.343 22 5 22H14C15.657 22 17 20.657 17 19V10C17 8.343 15.657 7 14 7H5ZM4 10C4 9.448 4.448 9 5 9H14C14.552 9 15 9.448 15 10V19C15 19.552 14.552 20 14 20H5C4.448 20 4 19.552 4 19V10Z"
                      fill="#B0B0B0"
                    />
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M9.942 2C8.65 2 7.504 2.826 7.096 4.051L7.051 4.184C6.877 4.708 7.16 5.274 7.684 5.449C8.208 5.623 8.774 5.34 8.949 4.816L8.993 4.684C9.129 4.275 9.511 4 9.942 4H19C19.552 4 20 4.448 20 5V14.059C20 14.489 19.725 14.871 19.316 15.007L19.184 15.051C18.66 15.226 18.377 15.792 18.551 16.316C18.726 16.84 19.292 17.123 19.816 16.949L19.949 16.904C21.174 16.496 22 15.35 22 14.059V5C22 3.343 20.657 2 19 2H9.942Z"
                      fill="#B0B0B0"
                      style={{
